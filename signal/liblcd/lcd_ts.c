...
static void lcd_ts_read(uint8_t addr, uint8_t *val)
{
	...
	spi_write(LCD_SPI,data,2);
#else
	spi_transmit_receive(LCD_SPI,data,2);
#endif
	LCD_TS_DIS;
	*val = data[1];
}
...
#else
	spi_transmit_receive(LCD_SPI,data,3);
#endif
	LCD_TS_DIS;
	*val = (data[1]<<8)|data[2];
}
 ...
 ****************************************************************/
/*
uint8_t ReadReg8(uint8_t addr, uint8_t *val);
...
//		data[0] = 0xD2; data[1] = 0xD3; data[2] = 0xD4; data[3] = 0xD5;
	...
	*z = (uint8_t)(sum_sample_z / samples);
	term_printf("x=%d, y=%d, z=%d, samples=%d\r\n",*x,*y,*z,samples);
#else
...
void lcd_ts_init(void)
{
	...
	// LCD_TS = PA9
	io_configure(LCD_TS_CS_PORT,LCD_TS_CS_PINS,LCD_TS_CS_CFG,NULL);
	LCD_TS_DIS;
