...
void lcd_draw_point(int x, int y, uint16_t color) 
{
	...
	spi_dma_write16_n(LCD_SPI, color, 1);
	LCD_DPY_DIS;
}
...
#define DMA_DEF_LEN		10000

void lcd_fill_rect(int x, int y, int w, int h, uint16_t color) {
  /*! \todo: should change endianess in Interface control (0xF6)? */
//	color = (color>>8)|(color<<8); /* swap */
	LCD_DPY_EN;
	lcd_set_window((uint16_t)x, (uint16_t)y, (uint16_t)(x+w-1), (uint16_t)(y+h-1));
#if 0
	for(int i=0; i<w*h; i++) {
		spi_write(LCD_SPI, (uint8_t*)&color, 2);
	}
#else
	size_t n_to_write=w*h;
	while (n_to_write) {
		size_t xfer_size = (n_to_write<DMA_DEF_LEN) ? n_to_write : DMA_DEF_LEN;
		spi_dma_write16_n(LCD_SPI, color, xfer_size);
		n_to_write -= xfer_size;
	}
#endif
	LCD_DPY_DIS;
}
...
void lcd_draw_line(int x0, int y0, int x1, int y1, uint16_t color, uint16_t width)
{
	if (x0>x1) {
	...
	if (y0 == y1) {		// horizontal line
		lcd_fill_rect(x0,y0,x1-x0+1,width,color);
		return;
	}
	if (x0 == x1) {		// vertical line
		lcd_fill_rect(x0,y0,width,y1-y0+1,color);
		return;
	}
...
void lcd_draw_rect(int x, int y, int w, int h, uint16_t color)
{
	...
	if (!h) h=1;
	LCD_DPY_EN;
	lcd_set_window((uint16_t)x, (uint16_t)y, (uint16_t)(x+w-1), (uint16_t)y);
	spi_dma_write16_n(LCD_SPI, color, w);
	lcd_set_window((uint16_t)x, (uint16_t)(y+h-1), (uint16_t)(x+w-1), (uint16_t)(y+h-1));
	spi_dma_write16_n(LCD_SPI, color, w);
	lcd_set_window((uint16_t)x, (uint16_t)(y+1), (uint16_t)x, (uint16_t)(y+h-1));
	spi_dma_write16_n(LCD_SPI, color, h-1);
	lcd_set_window((uint16_t)(x+w-1), (uint16_t)(y+1), (uint16_t)(x+w-1), (uint16_t)(y+h-1));
	spi_dma_write16_n(LCD_SPI, color, h-1);
	LCD_DPY_DIS;
}
/*
 ...
 */
void lcd_draw_line(int x1, int y1, int x2, int y2, Colour c)
 ...
 * \return void
 */
static void lcdDrawCircleHelper(int x0, int y0, int r, uint8_t cornername, uint16_t color)
{
	int f = 1 - r;
	int ddF_x = 1;
	int ddF_y = -2 * r;
 ...
 * \return void
 */
void lcd_draw_round_rect(int x, int y, int w, int h, int r, uint16_t color)
{
	// smarter version
...
uint16_t lcd_draw_char(DC *dc, int16_t x, int16_t y, char c)
{
	...
	spi_dma_write16(LCD_SPI, cache, cw*ch);
	LCD_DPY_DIS;
#else
	/*      mode     | MY MX MV ML BGR MH  0  0 |  hex
	   PORTRAIT      |  0  1  0  0   1  0  0  0 | 0x48
...
#endif
static int lcd_draw_raw_char(DC *dc, int x, int y, char c)
{
	int cw = dc->font->width, ch = dc->font->height;
...
*/
	...
	/* send the char color buffer to the screen */
	lcd_set_window((uint16_t)x, (uint16_t)y, (uint16_t)(x+cw-1), (uint16_t)(y+ch-1));
	spi_dma_write16(LCD_SPI, cache, cw*ch);
	
	return cw;
}

void lcd_draw_string(DC *dc, int x, int y, const char *s)
{
	uint16_t width = (uint16_t)(strlen(s)*dc->font->width);
...
void lcd_get_string_size(DC *dc, const char *s, int *width, int *height)
{
	switch (dc->tflags & DIR_MSK) {
	case DIR_HORIZONTAL:
	case DIR_HORIZONTAL_INV:
...
{	
	...
	// LCD_CS = PB6
	io_configure(LCD_DPY_CS_PORT,LCD_DPY_CS_PINS,LCD_DPY_CS_CFG,NULL);
	// LCD_BL = PC7
	io_configure(LCD_DPY_BL_PORT,LCD_DPY_BL_PINS,LCD_DPY_BL_CFG,NULL);
	// LCD_DC = PA8
	io_configure(LCD_DPY_DC_PORT,LCD_DPY_DC_PINS,LCD_DPY_DC_CFG,NULL);
	// SPI1 SCK = PA5, MISO = PA6, MOSI = PA7
	// SPI pins : SCK   --> D13 --> PA5 : SPI1_SCK
	//            MISO  --> D12 --> PA6 : SPI1_MISO
	//            MOSI  --> D11 --> PA7 : SPI1_MOSI
	freq=spi_master_init(LCD_SPI, LCD_DPY_CLOCK, LCD_DPY_CFG);
