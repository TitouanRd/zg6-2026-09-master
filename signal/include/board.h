...
/* type definitions for the STM32 microcontroller peripherals
 *	typedef ADC_t;					// Analog to Digital Converter
 *	typedef ADC_Common_t;
 *	typedef CRC_t;					// CRC calculation unit
 *	typedef DBGMCU_t;				// Debug MCU
 *	typedef DMA_Stream_t;			// DMA Controller
 *	typedef DMA_t;
 *	typedef EXTI_t;					// External Interrupt/Event Controller
 *	typedef FLASH_t;				// FLASH Registers
 *	typedef GPIO_t;					// General Purpose I/O
 *	typedef SYSCFG_t;				// System configuration controller
 *	typedef I2C_t;					// Inter-integrated Circuit Interface
 *	typedef IWDG_t;					// Independent WATCHDOG
 *	typedef PWR_t;					// Power Control
 *	typedef RCC_t;					// Reset and Clock Control
 *	typedef RTC_t;					// Real-Time Clock
 *	typedef SDIO_t;					// SD host Interface
 *	typedef SPI_t;					// Serial Peripheral Interface
 *	typedef TIM_t;					// Timer
 *	typedef	PWM_t;					// Timer redefinition
 *	typedef USART_t;				// Universal Synchronous Asynchronous Receiver Transmitter
 *	typedef WWDG_t;					// Window WATCHDOG

 *	typedef USB_OTG_Global_t;		// USB OTG Core registers
 *	typedef USB_OTG_Device_t;		// USB OTG device registers
 *	typedef USB_OTG_INEndpoint_t;	// IN_Endpoint-Specific_Register
 *	typedef USB_OTG_OUTEndpoint_t;	// OUT_Endpoint-Specific_Registers
 *	typedef USB_OTG_Host_t;			// Host_Mode_Register_Structures
 *	typedef USB_OTG_HostChannel_t;	// Host_Channel_Specific_Registers
 */
