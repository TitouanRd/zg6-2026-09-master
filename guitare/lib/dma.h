...
typedef struct _DMAEndPoint {
	Peripheral	type;			/* peripheral type (info) */
	const void*	addr0;			/* DR or buffer address */
	const void*	addr1;			/* second buffer address (double buffering) */
	int			channel;		/* channel request, -1 for memory buffer */
	uint32_t	cfg;			/* DR or buffer address attributes */
} DMAEndPoint_t;
...
/* configure a stream with 'mode' (priority and FIFO) attributes to connect 
 * 'src' and 'dest' endpoints. If 'cb' is non NULL, an IRQ is raised when the
 * transfer is complete
 */
DMA_Stream_t* dma_stream_init(DMA_t *dma, uint32_t stream, DMAEndPoint_t *src, DMAEndPoint_t *dest, uint32_t mode, OnTC cb);

/* start/stop to transfer the 'size' bytes */
...
int dma_get_count(DMA_Stream_t *s);
